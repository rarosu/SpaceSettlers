define(function(require) {
    ...
    var Keycodes = require('Components/Keycodes');
    ...
    CameraProcessor.prototype.update = function()
    {
        ...
        if (cameraEntity !== undefined)
        {
            ...
            if (inputReceiver.currentState.keyCodes.indexOf(Keycodes.KEYCODE_W) !== -1) {
                camera.lookAt.addScaledVector(forward, speed);
                camera.camera.position.addScaledVector(forward, speed);
            }

            if (inputReceiver.currentState.keyCodes.indexOf(Keycodes.KEYCODE_S) !== -1) {
                camera.lookAt.addScaledVector(forward, -speed);
                camera.camera.position.addScaledVector(forward, -speed);
            }

            if (inputReceiver.currentState.keyCodes.indexOf(Keycodes.KEYCODE_D) !== -1) {
                camera.lookAt.addScaledVector(right, speed);
                camera.camera.position.addScaledVector(right, speed);
            }

            if (inputReceiver.currentState.keyCodes.indexOf(Keycodes.KEYCODE_A) !== -1) {
                camera.lookAt.addScaledVector(right, -speed);
                camera.camera.position.addScaledVector(right, -speed);
            }
